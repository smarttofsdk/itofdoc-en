...
* Pixel calibration for depth data correction ::
	...
	dmcam.filter_enable(dev, dmcam.ITOF_FILTER_ID_PIXEL_CALIB, drnu_param, 0)	#enable pixel calibration
	
	dmcam.filter_disable(dev, dmcam.ITOF_FILTER_ID_PIXEL_CALIB)	#disable pixel calibration
	
* Depth filtering for depth data filtering::

	filter_param = dmcam.filter_args_u()
	filter_param.median_ksize = 3	
	dmcam.filter_enable(dev, dmcam.ITOF_FILTER_ID_MEDIAN, filter_param, sys.getsizeof(filter_param))	#enable depth filter
	
	dmcam.filter_disable(dev, dmcam.ITOF_FILTER_ID_MEDIAN)	#disable depth filter
...
* Amplitude filtering for filtering poor quality points::
	...
	dmcam.filter_enable(dev, dmcam.ITOF_FILTER_ID_AMP, amp_min_val, sys.getsizeof(amp_min_val))    #enable amplitude filter
	
	dmcam.filter_enable(dev, dmcam.ITOF_FILTER_ID_AMP)		#disable amplitude filter
...
* Automatic integration time, open the module to automatically adjust the exposure time according to the measured object::
	...
	dmcam.filter_enable(dev, dmcam.ITOF_FILTER_ID_AUTO_INTG, intg_auto_arg, sys.getsizeof(intg_auto_arg))  #Turn on automatic exposure
	
	dmcam.filter_disable(dev,ITOF_FILTER_ID_AUTO_INTG)		#Turn off automatic exposure
...
* sport mode 0，set frame format to 2(Only for TC-E series)::
	...
	dmcam.filter_enable(dev, dmcam.ITOF_FILTER_ID_SPORT_MODE, dmfilter, 0)		#open sport mode 0
	
	dmcam.filter_disable(dev, dmcam.ITOF_FILTER_ID_SPORT_MODE)		#close sport mode 0
...
* sport mode 1，set frame format to 2(Only for TC-E series)::
	...
	dmcam.filter_enable(dev, dmcam.ITOF_FILTER_ID_SPORT_MODE, dmfilter, 1) 	#open sport mode 1
	...
	dmcam.filter_disable(dev, dmcam.ITOF_FILTER_ID_SPORT_MODE)		#close sport mode 1
